...
async function renderEventCards() {
    ...
    // Rendre invisible pendant le rendu
    const cardsContainer = document.createElement('div');
    cardsContainer.id = 'contain-card';
    cardsContainer.className = 'row g-4 justify-content-center';
    cardsContainer.style.opacity = '0';
    
    // Créer les cartes
    events.forEach(event => {
        cardsContainer.innerHTML += createEventCardForGuests(event);
    });
    ...
    // Cacher le loader APRÈS avoir inséré les cartes
    if (loader) {
        loader.style.display = 'none';
    }
    ...
    requestAnimationFrame(() => {
        ...
        setTimeout(() => {
            const cards = cardsContainer.querySelectorAll('.event-card-pro');
            cards.forEach((card, i) => {
                setTimeout(() => {
                    card.classList.add('glow-animation');
                    setTimeout(() => card.classList.remove('glow-animation'), 800);
                }, i * 100);
            });
        }, 500);
    });
...
async function viewGuestQR(id) {
    ...
    const initials = `${guest.firstName[0] != '-' ? guest.firstName[0] : '' }${guest.lastName[0] != '-' ? guest.lastName[0] : '' }`.toUpperCase();
    const name = `${guest.firstName != '-' ? guest.firstName : '' } ${guest.lastName != '-' ? guest.lastName : '' }`;
